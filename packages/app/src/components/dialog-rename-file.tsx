...
export function DialogRenameFile(props: {
  ...
  const titleKey = () =>
    props.kind === "directory" ? "dialog.directory.rename.title" : "dialog.file.rename.title"
    ...
    <Dialog fit title={language.t(titleKey())} class="w-full max-w-[420px] mx-auto">
      <form onSubmit={handleSubmit} class="flex flex-col gap-6 p-6 pt-0">
        <TextField
          autofocus
          type="text"
          label={language.t("dialog.file.rename.name.label")}
          value={name()}
        ...
        <div class="flex justify-end gap-2">
          ...
          <Button
            ...
            variant="primary"
            size="large"
            disabled={submitting() || !name().trim() || name().trim() === props.currentName}
          >
            {submitting() ? language.t("common.saving") : language.t("dialog.file.rename.confirm")}
          </Button>
        </div>
      </form>
    </Dialog>
